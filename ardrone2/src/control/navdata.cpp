...
SHeliData helidata;
ros::Publisher *nav_publisher=NULL;
					...
					if ( get_mask_from_state( mykonos_state, MYKONOS_NAVDATA_DEMO_MASK ))
					{
						...
						if( cks == navdata_cks)
						{
							...
							helidata.vz = (double)navdata_unpacked.navdata_demo.vz;
                                                        if (nav_publisher) {
                                                           ar_drone2::Navdata nav;
                                                           nav.position.angular.x = helidata.phi;
                                                           nav.position.angular.y = helidata.psi;
                                                           nav.position.angular.z = helidata.theta;
                                                           nav.position.linear.x = helidata.vx;
                                                           nav.position.linear.y = helidata.vy;
                                                           nav.position.linear.z = helidata.vz;
                                                           nav.altitude = helidata.altitude;
                                                           nav.battery = helidata.battery;

                                                           //navdata publishing
                                                           nav_publisher->publish(nav); 
                                                        }
                                                }else{
                                                   INFO ("CKS Mismatch\n");
                                                }
